...
void random_test(size_t source_base_size ) {
    ...
    mxmz::ring_buffer   rb(1024 + rand_int() % (source_base_size/10));
    auto source = make_random_string(source_base_size  + rand_int() % 1000);
//    CERR << source << endl;
...
//                    CERR << ">" << buffer_size(buff) << endl;
                    size_t chunk = std::min( buffer_size(buff), std::min(  size_t(64 + rand_int() % 63),  size_t(source.end() - i ) ) );
   ...
   //                 CERR << "sleeping ...  source read = " << (i - source.begin() )  << endl;
                    CERR << "\rt1: sleeping                   ";
                    std::this_thread::sleep_for( std::chrono::milliseconds(rand_int() % 100 ));
                }
            }
            CERR << endl;
            CERR << "t1: end" << endl; 
