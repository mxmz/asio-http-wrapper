...
int main() {
	...
	dst.connect(
			asio::ip::tcp::endpoint(
					asio::ip::address::from_string("192.168.69.23"), 9002));
	//dst.connect( asio::ip::tcp::endpoint( asio::ip::address::from_string("127.0.0.1"), 9002) );

	auto pipe = std::make_shared<tcp_socket_pipe>(std::move(src),
			std::move(dst));

	pipe->run(
			[pipe](const system::error_code& read_ec, asio::ip::tcp::socket&& read_socket, const system::error_code& write_ec, asio::ip::tcp::socket&& write_socket) {
